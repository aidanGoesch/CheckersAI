...
void CheckersBoard::Draw() {
    ...
    // Iterate over the board and print each row
    for (const auto& row : m_Board) {
        ...
        for (const auto& e : row) {
            ...
            if (e.selected) {
                colorPair = 2;  // Selected background
            }

            // Set color pair and print piece
            if (colorPair != 0) {
                attron(COLOR_PAIR(colorPair));
            }
            printw("%s", piece.c_str());
            if (colorPair != 0) {
        ...
        switch (ch) {
            case 119:
                ...
                if (m_Selected.first > 0) {
                    ...
                    m_Board[m_Selected.first][m_Selected.second].selected = true;
                }
                break;
            case 115:
            std::cout << "something happened" << std::endl;
            ...
            std::cout << "something happened" << std::endl;
                ...
                break;
            case 10:
                // file << "selectingMove = " << selectingMove << "  m_Selected = " << m_Selected.first << " " << m_Selected.second << std::endl;
                if (selectingMove && m_Board[m_Selected.first][m_Selected.second].highlighted) // if you select a valid move
                    return false;
                if (!selectingMove && m_Board[m_Selected.first][m_Selected.second].player == PLAYER) 
                {  
                    m_ToMove = m_Selected;
                    return false;
                }
                if (selectingMove) // if you don't select a valid move
                    return true;
                break;
        }
...
{
    file << "number of children: " << m_RootNode->m_ChildNodes.size() << std::endl;
...
bool CheckersBoard::makeRandomMove(std::unordered_map<int, std::unordered_map<std::pair<int, int>, CompSquare*, pair_hash> >& pieceMap, const int& y0 = -1, const int& x0 = -1)
{
    ...
    if (y0 == -1 && x0 == -1)  // check for default params for chaining
    {
        ...
        {
            if (j.second != nullptr)
                file << j.second->coordinate.first << " " << j.second->coordinate.second << " " << j.second->possibleMoves.size() << std::endl;
        }
